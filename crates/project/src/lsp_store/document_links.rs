...
pub type DocumentLinkResolveTask = Shared<Task<Option<LspDocumentLink>>>;
...
pub(super) struct DocumentLinksData {
    ...
    pub(super) link_resolves: HashMap<(LanguageServerId, Range<Anchor>), DocumentLinkResolveTask>,
}
...
impl LspStore {
    ...
    pub fn fetch_document_links(
            ...
            && !version_queried_for.changed_since(&lsp_data.buffer_version)
        {
            ...
            if !has_different_servers {
                return Task::ready(Some(cached.links.values().flatten().cloned().collect()));
            }
        }
                ...
                lsp_store
                    .update(cx, |lsp_store, cx| {
                        ...
                        if lsp_data.buffer_version == query_version {
                            ...
                            // those servers so callers re-issue against the fresh ranges.
    ...
    ///
    /// Returns `None` when the cache no longer holds a matching link (likely a
    ...
    pub fn resolved_document_link(
        ...
        let cached_link = document_links
            .links
            .get(&server_id)?
            .iter()
                    ...
                    .update(cx, |lsp_store, _| {
                        ...
                        match resolved {
                            Some(resolved) => lsp_store
                                .cache_resolved_link(buffer_id, server_id, &range, &resolved),
                            None => {
    ...
    fn resolve_document_link_request(
        ...
        let buffer_id = buffer.read(cx).remote_id();
        let server_id = cached_link.server_id;
    ...
    fn cache_resolved_link(
        ...
        let document_links = self.lsp_data.get_mut(&buffer_id)?.document_links.as_mut()?;
        let links = document_links.links.get_mut(&server_id)?;
        let link = links.iter_mut().find(|l| l.range == *range)?;
